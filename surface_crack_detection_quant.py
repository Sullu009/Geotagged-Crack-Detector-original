...
TELEGRAM_CHAT_ID = 5257066612  # your chat ID
...
def inferencing(interpreter, queueIn, queueOut):
    ...
    while True:
        ...
        try:
            ...
            # --- NEW: ask GPS only when crack is detected ---
            if pred_class == 1:
                # Try to get GPS fix, but don't block forever
            ...
            if not queueOut.full():
                # now send gps_text to display() too
                queueOut.put((img, pred_class, pred_score, dp_out, gps_text))
...
def display(queueOut):
    ...
    alert_image_path = os.path.join(script_dir, "crack_alert.jpg")
    ...
    while True:
        ...
        start_time = time.time()
        img, pred_class, pred_score, dp_out, gps_text = queueOut.get()
        ...
        final_img = cv2.putText(final_img, gps_text, (980, 600), font, 1.2, (0, 0, 0), 3, cv2.LINE_AA)
        # --- Telegram alert when crack detected ---
        try:
            if label == "Crack" and pred_score > 0.7:
                cv2.imwrite(alert_image_path, final_img)
...
if __name__ == '__main__':
    ...
    model_file = '/home/shikhar/surface_crack/surface_crack_detection/model/quantized-model.lite'
